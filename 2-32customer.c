...
int customing(const char *myfifo, int need){
	...
	if((fd = open(myfifo, O_RDONLY)) < 0){
		perror("Function customing error");
		return -1;
	}
...
/*功能：从sour_file文件的offset偏移处开始将count字节数据复制到dest_file文件*/
int myfilecopy(const char *sour_file, const char *dest_file, int offset, int count, int copy_mode){
	...
	if((in_file = open(sour_file, O_RDONLY|O_NONBLOCK)) < 0){
		perror("Function myfilecopy error int source file\n");
		return -1;
	}

	if((out_file = open(dest_file, O_RDWR|O_CREAT|O_TRUNC|O_NONBLOCK, 0664)) < 0){
		perror("Function myfilecopy error in destination file");
		return -1;
	}
...
int custom(int need){
	...
	if((fd = open(fifo_file, O_RDWR)) < 0){
		perror("Function myfilecopy error in source_file");
		return -1;
	}
